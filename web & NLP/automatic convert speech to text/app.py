...
def index():
    ...
    database = db.execute("SELECT * FROM sound WHERE user_id = ? ", user_id)
...
def new():
    ...
    if request.method == "GET":
        ...
        symbol = request.form.get("output")
        if not symbol:
            return apology(" after speech ! ")


        user_id = session["user_id"]

        db.execute("INSERT INTO sound (user_id, words) VALUES (?, ?)",
                   user_id, symbol )
